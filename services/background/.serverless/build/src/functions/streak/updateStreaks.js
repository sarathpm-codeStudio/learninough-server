...
// src/functions/streak/updateStreaks.ts
var updateStreaks_exports = {};
__export(updateStreaks_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(updateStreaks_exports);
...
// src/modules/streak/streak.repository.ts
var ACTIVE_SECONDS_THRESHOLD = 60;
var IST_OFFSET_MS = 5.5 * 60 * 60 * 1e3;
var istDate = (d) => new Date(d.getTime() + IST_OFFSET_MS).toISOString().slice(0, 10);
var getSettingInt = async (key, fallback) => {
  const { data } = await supabaseAdmin.from("platform_settings").select("value").eq("key", key).maybeSingle();
  const n = parseInt(data?.value ?? "", 10);
  return Number.isFinite(n) ? n : fallback;
};
var streakRepository = {
  runDailyStreaks: async () => {
    ...
    const activeD = /* @__PURE__ */ new Map();
    const activeDm1 = /* @__PURE__ */ new Set();
    for (const r of rows ?? []) {
    ...
    if (activeIds.length) {
      const { data: profs, error: pErr } = await supabaseAdmin.from("profiles").select("id, role, streak_days, coin_balance").in("id", activeIds);
    ...
    for (const [uid, dRow] of activeD) {
      ...
      if (dRow.streak_day === true) continue;
      const continued = activeDm1.has(uid);
      const newStreak = continued ? (prof.streak_days ?? 0) + 1 : 1;
      const profileUpdate = { last_active: `${D}T12:00:00+05:30` };
      let rewarded = false;
      if (newStreak >= goal) {
        rewarded = true;
        ...
        const { error: cErr } = await supabaseAdmin.from("coin_transactions").insert({
          user_id: uid,
          type: "CREDIT",
          coin_count: reward,
      ...
      const { error: upErr } = await supabaseAdmin.from("profiles").update(profileUpdate).eq("id", uid);
      if (upErr) throw new Error(upErr.message);
      await supabaseAdmin.from("daily_analytics").update({ streak_day: true }).eq("user_id", uid).eq("date", D);
      if (rewarded) earned.push({ user_id: uid, coins: reward, days: goal });
    }
    const { data: streakers, error: sErr } = await supabaseAdmin.from("profiles").select("id, role, streak_days").eq("role", "STUDENT").gt("streak_days", 0);
    if (sErr) throw new Error(sErr.message);
    for (const s of streakers ?? []) {
      if (activeD.has(s.id)) continue;
      brokenIds.push(s.id);
    }
    if (brokenIds.length) {
      const { error: rErr } = await supabaseAdmin.from("profiles").update({ streak_days: 0 }).in("id", brokenIds);
    ...
    if (jobs.length) {
      const { error: insErr } = await supabaseAdmin.from("scheduled_notifications").insert(jobs);
