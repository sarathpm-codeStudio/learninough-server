...
var videoWebhookRepository = {
  handleVideoWebhook: async (data) => {
    try {
      ...
      const videoStatus = status === "ready" ? "READY" /* READY */ : status === "failed" ? "FAILED" /* FAILED */ : "PROCESSING" /* PROCESSING */;
      const { data: updatedMaterial, error } = await supabase.from("course_materials").update({
        material_status: videoStatus,
        transcoding_progress: percent_done
      }).eq("video_asset_id", asset_id).select().single();
