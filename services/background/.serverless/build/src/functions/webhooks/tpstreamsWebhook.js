...
// src/functions/webhooks/tpstreamsWebhook.ts
var tpstreamsWebhook_exports = {};
__export(tpstreamsWebhook_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(tpstreamsWebhook_exports);
...
var videoWebhookRepository = {
  handleVideoWebhook: async (data) => {
    try {
      ...
      const { data: allVideos } = await supabase.from("course_materials").select("id, title, material_status").eq("course_id", courseId).eq("is_deleted", false).eq("type", "VIDEO");
      ...
      const failedVideos = [
        ...
        ...allVideos?.filter(
          (v) => v.material_status === "FAILED" /* FAILED */
        ) ?? [],
        // Failed intro video ✅
        ...introFailed ? [{
          id: courseId,
      ...
      const processingVideos = [
        ...
        ...allVideos?.filter(
          (v) => v.material_status !== "COMPLETED" /* COMPLETED */ && v.material_status !== "FAILED" /* FAILED */
        ) ?? [],
...
// src/modules/webhook/video.webhook.service.ts
...
//# sourceMappingURL=tpstreamsWebhook.js.map
